...
Mutation testing for reactive circuit graphs -- automatically injects faults into a `CircuitGraph`, runs generated autotests, and measures how many observable mutations your declared verification surface catches.
...
console.log(`Generated: ${result.generatedMutants}, unobserved: ${result.unobserved.length}, skipped: ${result.skipped.length}`);
console.log('Survived:', result.survived);
console.log('Missing oracles:', result.unobserved);
```
...
7. If exploration triggers any assertion violation, the mutation is **killed**. Broad structural/effect mutants with the same behavior signature as the baseline are classified as **equivalent** rather than survived.
8. The final score is `killed / (observable total - invalid - equivalent) * 100`. Unobserved candidates are a missing-oracle report, not a normal mutation survivor.
...
| `operators` | `'all' \| string[]` | semantic scored set | Which operator classes to apply. Omit for the default observable semantic set, pass `'all'` for every operator, or pass an array of operator name prefixes (e.g. `['negate', 'sever-edge']`) to restrict the set. |
...
| `killed` | `number` | Number of observable mutations detected by assertions. |
...
| `survived` | `Array<{ mutation: string; description: string }>` | List of observable mutations that were **not** caught. Each entry contains the mutation `name` (e.g. `"negate:nodeId"`) and a human-readable `description`. |
...
| `unobserved` | `Array<{ mutation: string; description: string; reason: string }>` | Selected-mode candidates with no path to any declared verification sink. These are reported as missing-oracle gaps and excluded from the score denominator. |
| `score` | `number` | Kill rate as a percentage over observable scored mutants after invalid/equivalent classification. `100` means every scored mutation was caught. |
